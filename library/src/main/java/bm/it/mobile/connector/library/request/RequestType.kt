package bm.it.mobile.connector.library.request

import bm.it.mobile.connector.library.ConnectorType
import bm.it.mobile.connector.library.IConnectorCallback

class RequestType(private val connectorType: ConnectorType) :
    IRequestType {

    override fun post(url: String, callback: IConnectorCallback) {
    }

    override fun put(url: String, callback: IConnectorCallback) {
    }

    override fun delete(url: String, callback: IConnectorCallback) {
    }

    override fun get(url: String, callback: IConnectorCallback) {
    }
}